...
void init_disasm() {
  ...
  cs_mode mode = MUXDEF(CONFIG_ISA_x86,      CS_MODE_32,
	...
	int ret = cs_open_dl(arch, mode, &handle); // 用确定的架构和模式初始化Capstone引擎
...
void disassemble(char *str, int size, uint64_t pc, uint8_t *code, int nbyte) {
  // 指令跟踪（ITRACE）阶段实际调用反汇编功能的接口
  cs_insn *insn;
  size_t count = cs_disasm_dl(handle, code, nbyte, pc, 0, &insn); // 反汇编
  assert(count == 1); // 成功反汇编了一条指令
  int ret = snprintf(str, size, "%s", insn->mnemonic); // 指令助记符复制到str
  if (insn->op_str[0] != '\0') {
    snprintf(str + ret, size - ret, "\t%s", insn->op_str); // 附加上操作数字符串
  }
  cs_free_dl(insn, count); // 释放内存
}
