...
void init_regex() { //初始化正则表达式引擎，处理rules[]中的正则表达式
  ...
  for (i = 0; i < NR_REGEX; i ++) { //POSIX C 的正则表达式编译函数regcomp
...
static bool make_token(char *e) {
  ...
  while (e[position] != '\0') { // 开始扫描字符串，还没扫描到结尾的'\0'
    /* 对 该位置的字符 逐一尝试 所有规则。 */
    for (i = 0; i < NR_REGEX; i ++) { // e 是接收的参数，需要被解析成为token的表达式
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) { // 匹配到一个规则，进入switch
        ...
        if (expr_debug){                   // 调试信息，动态精度（Dynamic Precision）
          Log("match rules[%d] = \"%s\" at position %d with len %d: %.*s",i, rules[i].regex, position, substr_len, substr_len, substr_start);
        }

        position += substr_len; // 匹配成功以后，更新扫描位置，准备扫描下一个token

        switch (rules[i].token_type) { // 处理token.type
          case TK_NOTYPE: // 忽略空格
            break;        //退出switch

          case TK_SUB: // 判断是负号还是减号
            //printf("Processing '-', TK_NEG=%d, TK_SUB=%d\n", TK_NEG, TK_SUB);
            // 前面是number，')',HEX，即为减号，注意到 这里是 不等于 这三种情况
            if (nr_token == 0 ||
            ...
            // 正常拷贝token.str，strncpy(dest, src, len)，减号就1个长度
            strncpy(tokens[nr_token].str, substr_start, 1);
            tokens[nr_token].str[1] = '\0'; // 字符串补个结束符
            nr_token++; // token个数
            break;      // 退出switch
    ...
    // 所有规则都无法匹配，循环已经结束，i的值为rule数组的最大 NR_REGEX
    if (i == NR_REGEX) {
...
static void debug_tokens() {
  ...
  // 这正是make_token处理得到的结果，tokens[i].type, tokens[i].str
...
static bool check_parentheses(int p, int q) {
  ...
  for (int i = p + 1; i < q; i++) {
    if (tokens[i].type == TK_LPAREN) count++; // 若匹配到(，记为内部括号
      ...
      /* 说明匹配到的)是外层括号之外的闭合，内层括号已经全部匹配好并且闭合了 */
      if (count == 0) return false; 
      count--;                      // 若匹配到 ) ，则闭合内部括号
    }
  }

  return count == 0; // 最后 count 只要是 0 就说明内部括号完全匹配，这是一个bool式子
}
    ...
    for (int i = p; i <= q; i++) { // 扫描token，找到最低优先运算符的分割点
        ...
        if (type == TK_SUB) { // 判断 负号 和 减号
            if (i == p ||                     // '-' 在token的开头
                ...
                continue; // 跳过 负号 的情况，如果是加减是可以作为最低优先运算符号
            }
        }
...
static sword_t eval(int p, int q) {
    ...
    if (p == q) { // 单个 token
        ...
        if (tokens[p].type == TK_REG) {                                        
            char reg_name[32];
            strcpy(reg_name, tokens[p].str + 1); // 跳过第一个字符$，复制reg的name
            bool success = true;
            // 接口函数用reg_name查询对应的值
            sword_t val = isa_reg_str2val(reg_name, &success); 
            if (!success) { // 是否查找成功，寄存器名字是否存在
                printf("Invalid register name: %s\n", reg_name);
                assert(0);
            }
            return (sword_t)val;     // 返回寄存器的值
        }
    ...
    // 找最低优先级的主运算符，用于将表达式拆分为左右子表达式进行递归计算
    int op = find_main_op(p, q);

    /* 若未找到主运算符，
     * 只有括号和负号的可能（参考find_main_op内部跳过的两个优先级运算情况），
     * 而上面调用了check_parentheses保证除去了括号，
     * 故此处if只有 负号 或者 空
     */
    if (op == -1) { // 处理一元负号
          if (tokens[p].type == TK_NEG) { 
            return -eval(p + 1, q); // 负号 放在外面
        }
